...
d1 = np.load('data_v3.npz')
d2 = np.load('data_v3b.npz')
...
np.savez('data_v3-combined.npz',
         states=combined_states,
         actions=combined_actions,
         positions=combined_positions,
         seed=d1['seed'])

print(f"v3a samples:      {d1['states'].shape[0]}")
print(f"v3b samples:      {d2['states'].shape[0]}")
print(f"combined samples: {combined_states.shape[0]}")
print(f"Saved data_v3-combined.npz")
